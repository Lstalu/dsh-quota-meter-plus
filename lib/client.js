...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---------- 屏幕右上角常驻悬浮胶囊（余额 + 本会话已花/剩余） ----------
		var TB = {
			// 顶层容器：fixed 右上角，不拦截鼠标（仅胶囊/弹层自身可点）
			root: { position: "fixed", top: "12px", right: "16px", zIndex: 900, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "5px", pointerEvents: "none", fontVariantNumeric: "tabular-nums" },
			pill: { pointerEvents: "auto", display: "inline-flex", alignItems: "center", gap: "7px", padding: "3px 10px", borderRadius: "999px", background: "var(--dsw-alias-bg-layer-2)", border: "1px solid var(--dsw-alias-border-l2)", boxShadow: "var(--dsw-shadow-lv1)", fontSize: "11px", lineHeight: "18px", color: "var(--dsw-alias-label-secondary)", whiteSpace: "nowrap" },
			...
			popover: { pointerEvents: "auto", display: "flex", flexDirection: "column", gap: "8px", padding: "12px 14px", borderRadius: "12px", background: "var(--dsw-alias-bg-layer-2)", border: "1px solid var(--dsw-alias-border-inverted)", boxShadow: "var(--dsw-shadow-lv3)", fontSize: "12px", lineHeight: "18px", color: "var(--dsw-alias-label-secondary)", width: "280px" },
		...
		function TopBadge(props) {
			...
			return react.createElement("div", { style: TB.root },
				...
				react.createElement("div", { style: TB.pill }, sessionNode),
				open
			...
			ctx.slots.inject("shell.overlay", function () {
				try {
					return ctx.slots.register({
						name: "shell.overlay",
						id: "quota-top-badge",
						// 右上角常驻悬浮胶囊（余额校准 + 本会话额度）
						order: 95
					}, TopBadge);
				} catch (err) {
					console.warn("[quota] slot shell.overlay (badge) unavailable, skipped:", err && err.message);
					return null;
				}
			});
		}
