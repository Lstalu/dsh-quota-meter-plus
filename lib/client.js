	...
	factory: (require) => {
		...
		if (typeof document !== "undefined" && !document.getElementById("quota-meter-flow")) {
			...
			flowStyle.textContent = "@keyframes quota-meter-badge { 0% { opacity: 0; transform: translateY(-3px); } 12% { opacity: 1; transform: translateY(0); } 78% { opacity: 1; } 100% { opacity: 0; } }"
				...
				+ " @keyframes quota-meter-fadein { 0% { opacity: 0; transform: translateY(-3px); } 100% { opacity: 1; transform: translateY(0); } }"
				+ " @keyframes quota-meter-breath { 0%, 100% { opacity: 0.8; } 50% { opacity: 1; box-shadow: 0 0 6px currentColor; } }";
		...
		var S = {
			...
			planBadge: { flex: "none", display: "inline-block", padding: "0 6px", borderRadius: "999px", background: "var(--dsw-alias-state-success-tertiary)", color: "var(--dsw-alias-state-success-primary)", fontSize: "11px", lineHeight: "16px", fontWeight: 600, animation: "quota-meter-badge 1.2s ease forwards" },
			...
			peakBadgeActive: { animation: "quota-meter-fadein .3s ease, quota-meter-breath 2s ease-in-out .3s infinite" },
			freeBadge: { flex: "none", display: "inline-block", padding: "0 6px", borderRadius: "999px", background: "var(--dsw-alias-state-success-tertiary)", color: "var(--dsw-alias-label-primary)", fontSize: "11px", lineHeight: "16px", border: "none", cursor: "pointer", animation: "quota-meter-fadein .3s ease" },
		...
		// ---------- 会话头部右侧常驻胶囊（余额 + 本会话已花/剩余） ----------
		var TB = {
			// 顶层容器：锚定在会话头部 utilities 槽位（内容区顶部右侧，避开右侧面板）；
			// 单行胶囊，弹层相对本容器定位
			root: { position: "relative", display: "inline-flex", alignItems: "center", pointerEvents: "none", fontVariantNumeric: "tabular-nums" },
			pill: { pointerEvents: "auto", display: "inline-flex", alignItems: "center", gap: "7px", padding: "2px 10px", borderRadius: "999px", background: "var(--dsw-alias-bg-layer-2)", border: "1px solid var(--dsw-alias-border-l2)", boxShadow: "var(--dsw-shadow-lv1)", fontSize: "11px", lineHeight: "18px", color: "var(--dsw-alias-label-secondary)", whiteSpace: "nowrap" },
			sep: { width: "1px", height: "12px", background: "var(--dsw-alias-border-l2)", margin: "0 2px" },
			pillStrong: { color: "var(--dsw-alias-label-primary)", fontWeight: 600 },
			btn: { pointerEvents: "auto", appearance: "none", border: "none", background: "none", padding: "0 2px", cursor: "pointer", color: "var(--dsw-alias-label-tertiary)", fontSize: "11px", lineHeight: "18px" },
			btnHover: { color: "var(--dsw-alias-label-primary)" },
			keyInput: { boxSizing: "border-box", width: "240px", height: "30px", padding: "0 10px", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: "8px", background: "var(--dsw-alias-bg-layer-1)", color: "var(--dsw-alias-label-primary)", fontSize: "12px", lineHeight: "18px", outline: "none" },
			popover: { position: "absolute", top: "calc(100% + 6px)", right: 0, zIndex: 1000, pointerEvents: "auto", display: "flex", flexDirection: "column", gap: "8px", padding: "12px 14px", borderRadius: "12px", background: "var(--dsw-alias-bg-layer-2)", border: "1px solid var(--dsw-alias-border-inverted)", boxShadow: "var(--dsw-shadow-lv3)", fontSize: "12px", lineHeight: "18px", color: "var(--dsw-alias-label-secondary)", width: "280px" },
			popTitle: { fontSize: "13px", fontWeight: 600, color: "var(--dsw-alias-label-primary)", margin: 0 },
			popHint: { margin: 0, fontSize: "11px", lineHeight: "16px", color: "var(--dsw-alias-label-tertiary)" },
			popRow: { display: "flex", alignItems: "center", gap: "8px" },
		};
		function TopBadge(props) {
			var useSessions = props.useSessions;
			var current = useSessions(function (s) { return s.current; });
			var stateHook = react.useState(null);        // /quota/state
			var state = stateHook[0]; var setState = stateHook[1];
			var balHook = react.useState(null);          // /quota/balance
			var bal = balHook[0]; var setBal = balHook[1];
			var openHook = react.useState(false);        // 设置弹层
			var open = openHook[0]; var setOpen = openHook[1];
			var keyDraftHook = react.useState("");
			var keyDraft = keyDraftHook[0]; var setKeyDraft = keyDraftHook[1];
			var cfgHook = react.useState(null);          // 最近一次 /quota/config 应答
			var cfg = cfgHook[0]; var setCfg = cfgHook[1];

			// 会话额度状态：10s 轮询（输入框上的额度条是 1s，这里保持轻量）
			react.useEffect(function () {
				...
				refresh();
				var timer = window.setInterval(refresh, 10000);
				return function () { alive = false; window.clearInterval(timer); };
			}, [current]);

			// 账户真实余额：60s 轮询 + 首次立即拉取（失败静默，可手动刷新）
			var refreshBalance = react.useCallback(async function () {
				try {
					var r = await window.fetch("/quota/balance").then(function (x) { return x.json(); });
					setBal(r);
				} catch (err) { setBal({ ok: false, reason: "fetch-failed" }); }
			}, []);
			react.useEffect(function () {
				refreshBalance();
				var timer = window.setInterval(refreshBalance, 60000);
				return function () { window.clearInterval(timer); };
			}, [refreshBalance]);

			var saveKey = async function () {
				try {
					var r = await window.fetch("/quota/config", {
						method: "POST",
						headers: { "content-type": "application/json" },
			...
			var resetBaseline = async function () {
				try {
					var r = await window.fetch("/quota/config", {
						method: "POST",
						headers: { "content-type": "application/json" },
			...
			// 独立「清除密钥」（带确认）：清空已保存的 DeepSeek API Key，余额转"未配置"
			var clearKey = async function () {
				if (!window.confirm("确定清除已保存的 DeepSeek API Key？清除后余额将无法显示，需重新填入 Key。")) return;
				try {
					var r = await window.fetch("/quota/config", {
						method: "POST",
						headers: { "content-type": "application/json" },
			...
			// 余额胶囊文案
			var balNode = null;
			if (bal === null) balNode = react.createElement("span", null, "余额 …");
			else if (bal.ok && Number.isFinite(bal.balance)) {
				balNode = react.createElement("span", null,
					"余额 ", react.createElement("span", { style: TB.pillStrong }, bal.currency === "CNY" ? "¥" + bal.balance.toFixed(2) : bal.balance),
					bal.realSpent > 0.004
						? react.createElement("span", { title: "基线 ¥" + bal.baseline.toFixed(2) + " 以来的实测消耗（官方口径）" }, " · 实测 -¥" + bal.realSpent.toFixed(2))
						: null
				);
			} else if (bal.reason === "no-key") {
				balNode = react.createElement("span", null, "余额未配置");
			} else {
				balNode = react.createElement("span", null, "余额获取失败");
			}

			// 会话额度文案
			var spent = Number(state && state.spent) || 0;
			var quota = (state && state.quota) || null;
			var sessionNode = react.createElement("span", null,
				"本会话 已花 ", react.createElement("span", { style: TB.pillStrong }, "¥" + fmt(spent)),
				quota !== null && quota > 0
					? react.createElement("span", null, " · 剩余 ", react.createElement("span", { style: TB.pillStrong }, "¥" + fmt4(Math.max(0, quota - spent))))
					: react.createElement("span", null, " · 未设额度")
			);

			return react.createElement("div", { style: TB.root },
				react.createElement("div", { style: TB.pill },
					balNode,
					react.createElement("span", { style: TB.sep }),
					sessionNode,
					react.createElement("button", { style: TB.btn, title: "立即刷新余额", onClick: refreshBalance }, "↻"),
					react.createElement("button", { style: TB.btn, title: "余额校准设置（API Key / 基线）", onClick: function () { setOpen(function (v) { return !v; }); } }, "⚙")
				),
				open
					? react.createElement("div", { style: TB.popover },
						react.createElement("p", { style: TB.popTitle }, "余额校准设置"),
						react.createElement("p", { style: TB.popHint }, "填入 DeepSeek 平台 API Key（与 DSH 配置中的密钥相同）。仅本机保存、仅发往 api.deepseek.com 查询官方余额，不会回显。留空保存会保留现有 Key；如需清除请点「清除密钥」。"),
						react.createElement("input", { style: TB.keyInput, type: "password", value: keyDraft, placeholder: "sk-…", onChange: function (e) { setKeyDraft(e.target.value); } }),
						react.createElement("div", { style: TB.popRow },
							react.createElement("button", { style: S.btnPrimarySm, onClick: saveKey }, "保存"),
							react.createElement("button", { style: S.btnOutlineSm, onClick: resetBaseline }, "重设基线"),
							react.createElement("button", { style: S.btnOutlineSm, onClick: clearKey }, "清除密钥"),
							react.createElement("span", { style: { flex: 1 } }),
							react.createElement("span", null, cfg !== null ? (cfg.hasKey ? "已配置" : "未配置") : "")
						)
					)
					: null
			);
		...
		function apply(ctx) {
			// [patched] 各槽位注册独立 try/catch：声明方插件未就绪时静默降级，
			// 避免注册失败拖垮整个客户端启动（fail-loud 见 dsh-client-ui-slots）。
			ctx.slots.inject("conversation.input.dock", function () {
				try {
					...
					console.warn("[quota] slot conversation.input.dock unavailable, skipped:", err && err.message);
					return null;
				}
			});
			ctx.slots.inject("shell.overlay", function () {
				try {
					...
					console.warn("[quota] slot shell.overlay (toast) unavailable, skipped:", err && err.message);
					return null;
				}
			});
			ctx.slots.inject("conversation.session.header.utilities", function () {
				try {
					return ctx.slots.register({
						...
						id: "quota-top-badge",
						// 会话头部右侧常驻胶囊（余额校准 + 本会话额度）；
						// 选 header 槽位而非 viewport 悬浮：避免被右侧面板覆盖
						order: 95
					}, TopBadge);
				} catch (err) {
					console.warn("[quota] slot conversation.session.header.utilities unavailable, skipped:", err && err.message);
					return null;
				}
			});
		}
